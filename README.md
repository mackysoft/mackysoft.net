...
この自動化は GitHub Actions 標準の `GITHUB_TOKEN` を使います。repository 側では `Settings > Actions > General` で次を有効にしてください。

- `Workflow permissions`: `Read and write permissions`
- `Allow GitHub Actions to create and approve pull requests`: 有効

- 上記設定が戻ると `Sync Activity` または `Merge Sync Activity` が `gh` の認証・権限エラーで失敗します。
- 失敗した場合は repository 設定を確認し、必要なら `Sync Activity` を手動実行してください。
